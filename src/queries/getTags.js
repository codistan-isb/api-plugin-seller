...
export default async function tags(
  ...
  // Apply filtering based on `filterations`
  if (Array.isArray(filterations) && filterations.length > 0) {
    const filterArrayRegex = filterations.map(value => ({
      $regex: _.escapeRegExp(value),
      $options: "i"
    }));
    searchFieldFilter.$or = filterArrayRegex.map(regex => ({ name: regex }));
  }
